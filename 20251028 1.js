...
function displayQuestion() {
  if (currentQuestionIndex < questions.length) {
    ...
    // 新增淡江大學連結按鈕
    let tkuBtn = createButton('淡江大學');
    tkuBtn.id('tku-btn');
    tkuBtn.class('option-btn');
    tkuBtn.size(400, 40);
    tkuBtn.style('display', 'block');
    tkuBtn.mousePressed(() => window.open('https://www.tku.edu.tw', '_blank'));
    
    // 設定淡江大學按鈕位置
    const centerX = width / 2;
    const btnW = 400;
    const lastBtnY = buttons[3].position().y;  // 獲取最後一個選項按鈕的Y座標
    tkuBtn.position(centerX - btnW / 2, lastBtnY + 52);  // 在最後一個選項下方顯示
